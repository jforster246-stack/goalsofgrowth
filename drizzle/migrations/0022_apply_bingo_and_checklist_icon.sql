...
CREATE POLICY "Users manage own bingo cells" ON public.bingo_cells
  FOR ALL TO authenticated
  USING (EXISTS (SELECT 1 FROM public.bingo_cards c WHERE c.id = card_id AND c.user_id = auth.uid()))
  WITH CHECK (EXISTS (SELECT 1 FROM public.bingo_cards c WHERE c.id = card_id AND c.user_id = auth.uid()));
